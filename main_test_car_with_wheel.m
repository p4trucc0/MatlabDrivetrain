...
cww = CarWithWheel(1200, .04, tyre_param, 0.5, .3, 2, .29, drt);

% Try to simulate starting from 0km/h after revving up.

cnt.gas_pedal = 1.0;
cnt.clc_pedal = 1.0;
cnt.gear_lever = 1; % first gear

dt = .01;

tv = [0:dt:10];
xv = zeros(length(tv), 3);

w_car_0 = 0.0;
w_eng_0 = rpm2rads(1000);
w_whe_0 = 0.0;
...
for ii = 1:length(tv)
    if (tv(ii) > 3.0)
        cnt.clc_pedal = 0.0; % release clutch
    end
    xv(ii, 1) = w_eng_c;
    xv(ii, 2) = w_whe_c;
    xv(ii, 3) = w_car_c;
    xv2 = cww.get_acc([w_eng_c, w_whe_c, w_car_c]);
    w_eng_c = w_eng_c + xv2(1) * dt;
    w_whe_c = w_whe_c + xv2(2) * dt;
    w_car_c = w_car_c + xv2(3) * dt;
end
